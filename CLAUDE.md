...
| Full-text search | `tantivy` 0.25 | BM25 inverted index, stemming, ranked search |
...
- **Tantivy 0.25 `CompactDocValue`:** `TantivyDocument::get_first()` now returns `Option<CompactDocValue>` instead of `Option<OwnedValue>`. To extract a string, use the `Value` trait: `retrieved.get_first(field).and_then(|v| v.as_str())`. Must import `tantivy::schema::Value` for this to work.
